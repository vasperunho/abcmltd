...
async function loadTeamsDropdown() {
    ...
    const data = (dataJson.content || []).sort((a, b) => {
...
function refreshStatisticsChart(playerData) {
	...
	let avgDataset=[{
		label: ( isTeamChart ? (window.currentTeamData.nickName || window.currentTeamData.name) : `Μέσος όρος` ),
		data: [
	...
	if(isTeamChart){
		...
		avgDataset.push({
			label: (window.currentTeamData.nickName || window.currentTeamData.name) + " (παθητικό)",
			data: [
	...
	if(window.currentTeamData.id !== 39 && isTeamChart){
		...
		avgDataset.push({
			label: (window.teamsData[39].nickName ||  window.teamsData[39].name),
			data: [
		...
		avgDataset.push({
			label: (window.teamsData[39].nickName ||  window.teamsData[39].name) + " (παθητικό)",
			data: [
...
function refreshShotHeatmap(allGameStats) {
    ...
    // });
	var mapSize = 400;
	let data = new Array(mapSize).fill(0).map(() => new Array(mapSize).fill(0));;
    allShots.forEach(shot => {
		data[parseInt(shot.x * mapSize)][parseInt(shot.y * mapSize)] = (data[parseInt(shot.x * mapSize)][parseInt(shot.y * mapSize)] || 0) + (shot.made ? 1 : -1);
    });
	// const weights = [100,100,100,100,100,80,75,70,65,60,55,50,30,15,5,2];
	const weights = [100,100,100,100,100,100,100,100,85,80,75,70,65];
	// const weights = [100,100,100,80,70,65,60,55,30,15,2];
...
function applyMedianFilter(grid) {
    ...
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
            ...
            // Παίρνουμε μια περιοχή 3x3 γύρω από το κελί
            ...
            // Ταξινομούμε τις τιμές
            neighbors.sort((a, b) => a - b);

            // Παίρνουμε τη μεσαία τιμή (διάμεσος)
...
function applyOptimizedDiffusion(initialGrid, weights) {
    ...
    // 1. Επεξεργασία ανά Πηγή (Source-centric approach)
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
            ...
            // Ορίζουμε το "τετράγωνο" γύρω από την πηγή που επηρεάζεται
            ...
            for (let tr = rStart; tr <= rEnd; tr++) {
                for (let tc = cStart; tc <= cEnd; tc++) {
                    ...
                    if (dist < maxRadius) {
                        // Linear Interpolation για το βάρος
...
// Χώρος αποθήκευσης για τις επεξεργασμένες παλέτες
const paletteCache = new Map();

function getParsedPalette(palette) {
    // Δημιουργούμε ένα μοναδικό κλειδί με βάση τα περιεχόμενα της παλέτας
...
function getInterpolatedColor(val, parsedPalette) {
    // 1. Εύρεση των δύο σημείων παρεμβολής
    let i = 0;
    // Επειδή η παλέτα είναι μικρή (π.χ. 5-10 χρώματα), το while είναι ταχύτατο
    ...
    // 2. Υπολογισμός fraction
    const range = p2.offset - p1.offset;
    const fraction = range <= 0 ? 0 : (val - p1.offset) / range;

    // 3. Γραμμική Παρεμβολή (Lerp)
    const r = (p1.color[0] + (p2.color[0] - p1.color[0]) * fraction) | 0; // bitwise OR για γρήγορο rounding
...
// // Δημιουργία του Worker
// const workerCode = `
    // self.onmessage = function(e) {
        // // Εδώ βάλε όλο τον κώδικα της συνάρτησης applyOptimizedDiffusion...
        // const result = someCalculation(e.data.initialGrid, e.data.weights);
        // self.postMessage(result);
    // };
    // function someCalculation(grid, weights) { /* ... */ }
// `;
// const blob = new Blob([workerCode], { type: 'application/javascript' });
// const worker = new Worker(URL.createObjectURL(blob));

function startCalculation(data, weights) {
    console.log("Ο υπολογιστής ξεκίνησε στο παρασκήνιο...");
    
    // Αποστολή δεδομένων στον Worker
    worker.postMessage({ initialGrid: data, weights: weights });

    // Λήψη αποτελεσμάτων
    worker.onmessage = function(e) {
        const { gridA, gridB, gridC } = e.data;
        console.log("Ο υπολογισμός ολοκληρώθηκε!");
        // Εδώ μπορείς να ενημερώσεις το UI σου με τον πίνακα gridC
    };
}
function smoothGrid(grid) {
    ...
    const cols = grid[0].length;
	const radius = 1;
    // Δημιουργούμε ένα αντίγραφο για να μην αλλοιώσουμε τα αρχικά δεδομένα κατά τη διάρκεια του υπολογισμού
...
function renderHeatmap(data, opt) {
    ...
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
            const val = data[r][c];
            // const val = data[r][c] === 0 ? 0 : ((data[r][c]) + 1) * 0.5;
            // Χρήση της παραμετροποιημένης παλέτας
			if(val === 0) continue;
